...
int main() {
	...
	glGenBuffers(1, &VBO);
	// specify only have 1 object for index buffer 
	glGenBuffers(1, &EBO);
	...
	// make the index buffer current (bind it) so it can be used
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	// link EBO to indices array
	...
	glBindVertexArray(0);
	// unbind the index buffer, make sure to do AFTER unbind VAO, bc EBO is stored in VAO
	...
	while(!glfwWindowShouldClose(window)) {
		...
		glBindVertexArray(VAO);
		// primitive we want to draw, num indices to draw, data type of indices, starting index 
		glDrawElements(GL_TRIANGLES, 9, GL_UNSIGNED_INT, 0);
		//REPLACED: glDrawArrays(GL_TRIANGLES, 0, 3);
